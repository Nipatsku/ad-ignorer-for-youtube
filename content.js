...
const Youtube = (() => {
  ...
  const getHTML_skipAdButton = () => {
    ...
    return (
      videoPlayerDiv &&
      (videoPlayerDiv.getElementsByClassName("ytp-ad-skip-button-text")[0] ||
        videoPlayerDiv.getElementsByClassName("ytp-skip-ad-button__text")[0])
    );
  };
...
const Intermission = (() => {
  const Asset = (fileName) => chrome.runtime.getURL(`/assets/${fileName}`);
  const cs_base_animate = cs_base + "_animation";

  const _removeAnimations = () => {
    const divs = [...document.getElementsByClassName(cs_base_animate)].filter(
  ...
  return (target, state = {}) => {
    ...
    div.style.opacity = 0;
    div.className =
      cs_base + " " + cs_base_animate + " " + cs_base_animate + "_brb";
...
(async () => {
  ...
  const iteration = (video, state) => {
    ...
    if (advertisement !== state.advertisement) {
      ...
      console.log(
        `Youtube Ad state changed to: ${advertisement ? "ON" : "OFF"}`
      );
